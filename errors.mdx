...
| 403 | `NOT_AUTHORIZED` | Login completed the email code, but the email may not enroll: it is not on WAT's resident list (or the account is deactivated). | Ask a WAT admin to add your email or company domain (web app, **Admin → Access**), then log in again. |

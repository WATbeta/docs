...
| `MISSING_FLAG` | `wat login --json` needs a flag it did not get (it never prompts in JSON mode). | Pass the missing flag: `--email`, `--code`, or `--signup-code` on first enrollment. |

...
<Steps>
  ...
  <Step title="Ask your human for the 6-digit code, then complete">
    ...
    ```
    If the email isn't on WAT's resident list, this returns `NOT_AUTHORIZED` — relay the message to your human: a WAT admin must add their email or company domain (web app, **Admin → Access**), then retry from step 2. On success an API key is minted and stored; you will not need to log in again on this machine.
  </Step>
</Steps>

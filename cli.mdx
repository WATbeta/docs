...
Every prompt has a flag, so scripts and agents can log in without a TTY:

```bash
wat login --email you@example.com --code 123456 --key-name "ci-agent"
```

| Flag | What it does |
|---|---|
| `--email <email>` | The account email (otherwise prompted). |
| `--code <code>` | The 6-digit code from the login email (otherwise prompted). |
| `--signup-code <code>` | The WAT signup code, required on first enrollment only. |
...
With `--json`, `wat login` never prompts: a missing required flag fails fast with a structured `MISSING_FLAG` error instead of hanging on input. See [Error codes](/errors).

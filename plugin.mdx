...
| `wat:book-room` | Creates a booking — optionally inviting guests by email — surfacing slot conflicts, the daily budget, and out-of-hours requests (the API rejects times outside 06:00 to 22:00 Europe/Brussels with `outside_hours`) cleanly. | "Book Room A tomorrow 10:00 to 11:00 and invite jane@client.com." |
| `wat:my-bookings` | Lists your own bookings (with their guest lists), reschedules one, changes its guests, or cancels one by id. | "Show my bookings and cancel the 3pm one." |
